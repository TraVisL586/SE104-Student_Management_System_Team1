...
export function CourseSections() {
  ...
  const getStatusConfig = (section) => {
    const isFull = section.status === "OPEN" && (section.enrolledCount || 0) >= (section.capacity || 1);
    if (section.status === "CLOSED") return { label: "Đã đóng", color: "#64748b", bg: "#f1f5f9" };
    if (section.status === "CANCELLED") return { label: "Đã hủy", color: "#dc2626", bg: "#fee2e2" };
    if (section.status === "DRAFT") return { label: "Nháp", color: "#7c3aed", bg: "#ede9fe" };
    if (isFull) return { label: "Đã đầy", color: "#f59e0b", bg: "#fef3c7" };
    return { label: "Đang mở", color: "#10b981", bg: "#d1fae5" };
  };

  const openCount = sections.filter(s => s.status === "OPEN" || !s.status).length;
  const fullCount = sections.filter(s => s.status === "OPEN" && (s.enrolledCount || 0) >= (s.capacity || 1)).length;
    ...
    <div className="space-y-5">
      ...
      >
              <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Số tiết / buổi</label>
                  <input
                    type="number"
                    min="1"
                    max="10"
                    value={form.periodsPerSession}
                    onChange={(e) => setForm(p => ({ ...p, periodsPerSession: Math.min(10, Math.max(1, parseInt(e.target.value) || 1)) }))}
                    className="w-full p-2 border rounded-lg text-sm"
                  />
                </div>
            ...
            <table className="w-full" style={{ minWidth: 800 }}>
              <thead>
                <tr style={{ backgroundColor: "#f8fafc" }}>
                  {["Mã lớp HP", "Môn học", "Giảng viên", "Số tiết", "Sĩ số", "Trạng thái", "Thao tác"].map((h) => (
                ...
                {filtered.length === 0 ? (
                  ...
                  return (
                    <tr key={s.id} style={{ borderTop: "1px solid #f1f5f9" }}>
                      ...
                      <td className="px-4 py-3" style={{ fontSize: "0.82rem", color: "#1e293b", fontWeight: 700 }}>{s.periodsPerSession || 1} tiết</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
